...
import { latestAvailableRun } from './ensemble-ingest';
...
export function inRunWindow(now: Date): boolean {
  for (const mdl of MODELS) {
    for (let back = 0; back < 30; back++) {
      ...
      if (!mdl.runHoursUtc.includes(h)) continue;
      ...
      if (delta >= -RUN_PAUSE_BEFORE_MIN * 60000 && delta <= RUN_PAUSE_AFTER_MIN * 60000) return true;
    }
  }
  return false;
}

export async function runMmQuoteSim(
  supabase: SupabaseClient,
  options: { verbose?: boolean } = {}
): Promise<{ quoted: number; fillsScored: number; resolvedScored: number; errors: number }> {
  const verbose = options.verbose ?? true;
  const log = (m: string) => { if (verbose) console.log(`[mm-sim] ${m}`); };
  ...
  const unscoredMarketIds = [...new Set((unscored ?? []).map((r) => String((r as { market_id: string }).market_id)))];
  const marketState = new Map<string, { mid: number | null; resolved: boolean; winnerYes: boolean | null }>();
  if (unscoredMarketIds.length > 0) {
    ...
    for (const m of mkts ?? []) {
      const r = m as { id: string; outcome_prices: number[] | null; is_resolved: boolean; resolution_val: string | null };
  ...
  for (const row of unscored ?? []) {
    const q = row as { id: number; market_id: string; quote_bid: number; quote_ask: number; size_usd: number; filled_side: string | null; fill_price: number | null };
  ...
  for (const row of analyses ?? []) {
    const a = row as { market_id: string; true_prob: number | null; market_price: number | null; flags: string[] | null };
    const mid = a.market_price;
    const p = a.true_prob;
    if (seen.has(String(a.market_id))) continue;
    seen.add(String(a.market_id));
    if (p == null || mid == null || mid < 0.03 || mid > 0.97) continue;
  ...
  if (quotes.length > 0) {
    ...
    if (insErr) { errors++; console.warn(`[mm-sim] insert: ${insErr.message}`); }
    else quoted = quotes.length;
  }
