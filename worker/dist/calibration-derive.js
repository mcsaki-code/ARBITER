...
exports.deriveCalibration = deriveCalibration;
 ...
 */
async function fetchAllRaw(supabase) {
    ...
    const { data: cities, error: citiesErr } = await supabase
        ...
        .limit(1000);
    if (citiesErr)
        throw new Error(`city list fetch: ${citiesErr.message}`);
    const cityIds = [...new Set((cities ?? []).map((r) => r.city_id))];
    ...
    const { data: allCities, error: allCitiesErr } = await supabase
        ...
        .eq('is_active', true);
    if (allCitiesErr)
        throw new Error(`weather_cities fetch: ${allCitiesErr.message}`);
    const activeCityIds = (allCities ?? []).map((c) => c.id);
    const all = [];
    for (const cityId of activeCityIds) {
        ...
        while (true) {
            ...
            if (error)
                throw new Error(`raw fetch city ${cityId}: ${error.message}`);
...
async function deriveCalibration() {
    ...
    const sigmaInserted = await upsertChunks(supabase, 'weather_calibration_sigma', sigmaRows, 'city_id,lead_days,month');
    console.log(`[calibration-derive] sigma: ${sigmaInserted} rows (range ${sigmaMin.toFixed(2)}°F – ${sigmaMax.toFixed(2)}°F)`);
    // ── BIAS: (city, source, lead_days, month) ──────────────────────────
    const biasGroups = groupBy(raw, (r) => `${r.city_id}|${r.source}|${r.lead_days}|${monthOf(r.valid_date)}`);
    const biasRows = [];
    ...
    const biasInserted = await upsertChunks(supabase, 'weather_calibration_bias', biasRows, 'city_id,source,lead_days,month');
    ...
    const weightsInserted = await upsertChunks(supabase, 'weather_calibration_weights', weightRows, 'city_id,source,lead_days');
    const minSum = Math.min(...weightSumCheck);
    const maxSum = Math.max(...weightSumCheck);
    console.log(`[calibration-derive] weights: ${weightsInserted} rows (sum range ${minSum.toFixed(4)} – ${maxSum.toFixed(4)})`);
...
//# sourceMappingURL=calibration-derive.js.map