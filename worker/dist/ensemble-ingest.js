...
exports.latestAvailableRun = latestAvailableRun;
exports.dailyMaxByMember = dailyMaxByMember;
exports.ingestEnsembleForecasts = ingestEnsembleForecasts;
...
function latestAvailableRun(model, now) {
    ...
    for (let back = 0; back < 48; back++) {
        ...
        if (model.runHoursUtc.includes(h)) {
            const runStart = new Date(Date.UTC(cand.getUTCFullYear(), cand.getUTCMonth(), cand.getUTCDate(), h));
...
function dailyMaxByMember(hourly) {
    ...
    for (let i = 0; i < times.length; i++) {
        ...
        for (let m = 0; m < memberKeys.length; m++) {
            const v = hourly[memberKeys[m]][i];
            if (v != null && Number.isFinite(v) && v > arr[m])
                arr[m] = v;
        }
    }
 ...
 */
async function ingestEnsembleForecasts(supabase, options = {}) {
    const start = Date.now();
    const verbose = options.verbose ?? true;
    const log = (m) => { if (verbose)
        console.log(`[ens-ingest] ${m}`); };
    ...
    if (cityErr || !cities?.length) {
        if (cityErr) {
            console.error(`[ens-ingest] cities fetch: ${cityErr.message}`);
            errors++;
        }
    ...
    const have = new Set((haveRows ?? []).map((r) => `${r.city_id}|${r.run_label}`));
    for (const mdl of MODELS) {
        const run = latestAvailableRun(mdl, now);
        const runLabel = run.runLabel(mdl.model);
        for (const city of cities) {
            if (have.has(`${city.id}|${runLabel}`)) {
                skippedUpToDate++;
                continue;
            }
            ...
            try {
                ...
                for (const [date, members] of byDate) {
                    ...
                    if (members.length < MIN_MEMBERS_TO_STORE)
                        continue;
                ...
                if (rows.length > 0) {
                    ...
                    if (insErr) {
                        errors++;
                        console.warn(`[ens-ingest] insert ${city.name}: ${insErr.message}`);
                    }
                    else
                        rowsInserted += count ?? rows.length;
                }
            }
            catch (e) {
                errors++;
...
//# sourceMappingURL=ensemble-ingest.js.map