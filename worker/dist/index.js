...
async function logHealth() {
    ...
    console.log(`[worker] ── Health ── uptime=${uptimeHours}h | ` +
        ...
        `cacheSize=${(0, monitor_1.getCacheSize)()}`);
...
async function runTemperatureLoop() {
    ...
    while (true) {
        try {
            ...
            const result = await (0, temperature_1.analyzeTemperatureMarkets)(supabase, {
...
async function runMonitorLoop() {
    ...
    while (true) {
        try {
            ...
            const { result, changedIds } = await (0, monitor_1.scanMarketPrices)(supabase);
            ...
            if (changedIds.length > 0) {
                ...
                const tempChangedIds = (changedMarkets ?? [])
                    .filter((m) => m.category === 'temperature')
                    .map((m) => m.id);
