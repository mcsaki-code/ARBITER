...
exports.writeShadowRow = writeShadowRow;
exports.scoreResolvedShadows = scoreResolvedShadows;
const MIN_EDGE_BET_THRESHOLD = 0.08;
const MIN_PRICE_BET_THRESHOLD = 0.05;
const MAX_PRICE_BET_THRESHOLD = 0.995;
function bucketLeadTime(hours) {
    if (hours < 6)
        return '0-6h';
    if (hours < 12)
        return '6-12h';
...
async function writeShadowRow(supabase, input) {
    try {
        const edge = input.marketPriceYes != null
            ? input.predictedProb - input.marketPriceYes
            : null;
        const wouldHaveBet = edge != null &&
        ...
        console.warn(`[backtest-shadow] write failed for ${input.marketId}: ${err instanceof Error ? err.message : String(err)}`);
    }
}
/**
 ...
 */
async function scoreResolvedShadows(supabase, options = {}) {
    const limit = options.limit ?? 1000;
    const verbose = options.verbose ?? false;
    const log = (m) => verbose && console.log(`[shadow-scorer] ${m}`);
...
//# sourceMappingURL=backtest-shadow.js.map