...
exports.loadCalibrationSnapshot = loadCalibrationSnapshot;
exports.lookupSigma = lookupSigma;
exports.lookupBias = lookupBias;
exports.lookupWeight = lookupWeight;
exports.blendSigma = blendSigma;
exports.debiasMembers = debiasMembers;
...
function sigmaKey(city_id, lead_days, month) {
    return `${city_id}|${lead_days}|${month}`;
}
function biasKey(city_id, source, lead_days, month) {
    return `${city_id}|${source}|${lead_days}|${month}`;
}
function weightKey(city_id, source, lead_days) {
 ...
 */
async function loadCalibrationSnapshot(supabase) {
    try {
        ...
        console.log(`[calibration-lookup] loaded: sigma=${snap.rowCounts.sigma} bias=${snap.rowCounts.bias} weights=${snap.rowCounts.weights}`);
        return snap;
    }
    catch (err) {
 ...
 */
function lookupSigma(snap, city_id, lead_days, month) {
    if (!snap.enabled)
        return null;
...
function lookupBias(snap, city_id, source, lead_days, month) {
    if (!snap.enabled)
        return null;
    const lead = clampLead(lead_days);
...
function lookupWeight(snap, city_id, source, lead_days) {
    if (!snap.enabled)
        return null;
    const lead = clampLead(lead_days);
 ...
 */
function blendSigma(sampleSigma, floorSigma, empiricalSigma, nCal) {
 ...
 */
function debiasMembers(members, snap, city_id, lead_days, month) {
...
//# sourceMappingURL=calibration-lookup.js.map