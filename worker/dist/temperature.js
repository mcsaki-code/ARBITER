...
const calibration_lookup_1 = require("./calibration-lookup");
...
function parseTemperatureQuestion(question) {
    ...
    const matchF = question.match(/highest temperature in ([A-Za-z\s\u00C0-\u024F]+?) be (\d+)°F( or higher| or lower| or above| or below)? on ([A-Za-z]+ \d+)/i);
    if (matchF) {
...
function sigmaCForDaysOut(daysOut) {
    if (daysOut <= 0)
        return 1.5; // same-day: tight ±1.5°C
    if (daysOut <= 1)
        return 2.5; // 1-day out: ±2.5°C
    if (daysOut <= 2)
        return 3.5; // 2-day out: ±3.5°C
    return 4.5; // 3-5 day out: ±4.5°C
}
async function analyzeTemperatureMarkets(supabase, options = {}) {
    ...
    const log = (msg) => { if (verbose)
        console.log(`[temp-analysis] ${msg}`); };
    const err = (msg) => console.error(`[temp-analysis] ERROR: ${msg}`);
    ...
    const calibration = await (0, calibration_lookup_1.loadCalibrationSnapshot)(supabase);
    ...
    const ensPool = new Map(); // `${city_id}|${valid_date}` → members °F
    {
        ...
        for (const row of ensRows ?? []) {
            ...
            if (seen.has(modelKey))
                continue; // keep only the latest run per model
            seen.add(modelKey);
    ...
    for (const market of tempMarkets) {
        ...
        const city = cityList.find(c => c.name.toLowerCase() === cityNameLc ||
        ...
        let prior;
        if (calibration.enabled) {
            const debiased = (0, calibration_lookup_1.debiasMembers)(members, calibration, city.id, leadDays, monthNum);
            const sigmaLookup = (0, calibration_lookup_1.lookupSigma)(calibration, city.id, leadDays, monthNum);
            ...
            for (const m of members) {
                const w = (0, calibration_lookup_1.lookupWeight)(calibration, city.id, m.source, leadDays);
                if (w) {
            ...
            if (weightSum > 0) {
                for (const [src, w] of weightEntries)
                    weightBySource.set(src, w / weightSum);
            }
            prior = {
        ...
        let nCalSigma = null;
        if (useEnsemble) {
            const ens = (0, forecast_ensemble_1.computeEnsembleEcdfProbability)(pool, bracket);
            ...
            const forecastProb = (0, forecast_ensemble_1.computeBracketProbability)(members, bracket, hoursRemaining, prior);
        ...
        await (0, backtest_shadow_1.writeShadowRow)(supabase, {
