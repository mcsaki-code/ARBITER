...
function getDynamicSigmaFloor(hoursRemaining) {
    if (hoursRemaining <= 6)
        return 1.1;
    ...
    if (hoursRemaining <= 48)
        return 5.0; // measured 8.8 w/ +7°F bias — data suspect, betting blocked here
    return 5.5;
}
// ── Sample statistics ─────────────────────────────────────────
...
// ── Main entrypoint ───────────────────────────────────────────
function computeBracketProbability(members, bracket, hoursRemaining, prior) {
    ...
    let sigmaSource = sampleStd >= sigmaFloor ? 'sample' : 'floor';
    if (prior?.empiricalSigmaF != null &&
        prior.nCalSigma != null &&
        prior.nCalSigma >= 20) {
        const alpha = Math.min(1, prior.nCalSigma / 30);
        const baseline = Math.max(sampleStd, sigmaFloor);
        blendedSigma = Math.sqrt(alpha * prior.empiricalSigmaF ** 2 + (1 - alpha) * baseline ** 2);
        sigmaSource = 'blend';
    }
    const calibrationUsed = !!(prior?.debiasedMembers ||
        ...
        if (prior?.weightBySource && prior.weightBySource.size > 0) {
            ...
            for (const m of workingMembers) {
                ...
                if (w <= 0)
                    continue;
                wTotal += w;
                if (inBracket(m.temp_high_f, bracket))
                    wHit += w;
            }
            if (wTotal > 0) {
                const epsilon = 1 / (n + 2);
...
function inBracket(v, b) {
    if (b.low_f !== null && v < b.low_f)
        return false;
    if (b.high_f !== null && v >= b.high_f)
        return false;
    return true;
}
// ── Normal approximation: P(low ≤ T < high) ────────────────────
function normalBracketProb(mean, sigma, b) {
    if (sigma <= 0)
        return inBracket(mean, b) ? 0.99 : 0.01;
...
function parseBracketFromQuestion(question) {
    if (!question)
        return null;
...
//# sourceMappingURL=forecast-ensemble.js.map