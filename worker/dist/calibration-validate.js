...
async function loadSigmaMap(sb) {
    const map = new Map();
    const PAGE = 1000;
    let from = 0;
    while (true) {
        ...
        if (error) {
            console.warn('[validate] sigma load error:', error.message);
            break;
        }
...
function calibrateProb(uncalProb, sampleSigma, leadHours, cityId, month, sigmaMap) {
    ...
    const { sigma: blended } = (0, calibration_lookup_1.blendSigma)(sampleSigma, floor, entry.sigma_f, entry.n);
...
async function main() {
    ...
    const { data: rows, error } = await supabase
        ...
        .select('id, market_id, city_id, predicted_prob, sigma_f, mean_f, n_members, ' +
    ...
    for (const row of rows) {
        ...
        if (row.city_id && row.lead_time_hours != null && row.sigma_f != null) {
            ...
            const { prob: calProb, hadCalData } = calibrateProb(uncalProb, Number(row.sigma_f), Number(row.lead_time_hours), row.city_id, month, sigmaMap);
...
//# sourceMappingURL=calibration-validate.js.map