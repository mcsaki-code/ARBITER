...
exports.ingestStationActuals = ingestStationActuals;
exports.parseResolutionStation = parseResolutionStation;
exports.backfillResolutionSources = backfillResolutionSources;
...
function localDateFor(utcMs, timezone) {
    try {
        if (timezone) {
...
async function ingestStationActuals(supabase, options = {}) {
    const verbose = options.verbose ?? true;
    const log = (m) => { if (verbose)
        console.log(`[station-actuals] ${m}`); };
    ...
    if (mapErr || !mapRows?.length) {
        if (mapErr) {
            console.error(`[station-actuals] map fetch: ${mapErr.message}`);
            errors++;
        }
    ...
    for (let i = 0; i < stationIds.length; i += METAR_BATCH) {
        ...
        try {
            ...
            if (!res.ok) {
                errors++;
                console.warn(`[station-actuals] HTTP ${res.status} for batch ${i / METAR_BATCH}`);
                continue;
            }
            const data = await res.json();
            const obs = Array.isArray(data) ? data : (data.metar ?? []);
            for (const ob of obs) {
                const sid = (ob.icaoId ?? '').toUpperCase();
                const meta = stationMeta.get(sid);
                if (!meta || ob.temp == null || !Number.isFinite(ob.temp))
                    continue;
                ...
                if (!cur || ob.temp > cur.maxC)
                    acc.set(key, { maxC: ob.temp, n: (cur?.n ?? 0) + 1 });
                else
                    cur.n++;
            }
        }
    ...
    if (upErr) {
        errors++;
        console.warn(`[station-actuals] upsert: ${upErr.message}`);
    }
    else
        daysUpserted = upserts.length;
...
function parseResolutionStation(description) {
    if (!description)
        return null;
    ...
    if (near)
        return near[1].toUpperCase();
    return null;
}
async function backfillResolutionSources(supabase, options = {}) {
    const verbose = options.verbose ?? true;
    const log = (m) => { if (verbose)
        console.log(`[res-source] ${m}`); };
...
//# sourceMappingURL=station-actuals.js.map