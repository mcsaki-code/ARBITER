  ...
  "flags": string[]
    ...
    try {
      ...
      if (analysis.direction !== 'PASS' && edgeNorm >= MIN_EDGE && kellyFraction > 0) {
        ...
        kellyFraction = Math.min(kellyFraction, 0.025); // Sentiment bets: slightly more conservative
        recBetUsd = Math.max(1, Math.round(bankroll * kellyFraction * confMult * 100) / 100);
        recBetUsd = Math.min(recBetUsd, bankroll * 0.02); // Cap at 2% for sentiment bets
      }
      ...
      await supabase.from('sentiment_analyses').insert({
        ...
        flags:            analysis.flags ?? [signal.type],
      });
      ...
      console.log(
        ...
        `edge=${edgeNorm.toFixed(3)} dir=${analysis.direction} conf=${analysis.confidence} $${recBetUsd}`
      );
